...
from epftoolbox.models._lear_original import LEAR
from epftoolbox.models._lear_original import LEAR_adaptive_normalization as LEAR_as
...
def process_combination(combination):
    ...
    if apply_adaptive_standardisation:
        forecast.to_csv(f"Results_py//dataset_{dataset.replace('.csv', '')}_larsmodel_LEAR_as_calibration_window{calibration_window}.csv")
    else:
        forecast.to_csv(f"Results_py//dataset_{dataset.replace('.csv', '')}_larsmodel_LEAR_calibration_window{calibration_window}.csv")
    forecast = pd.DataFrame(index=df_test.index[::24], columns=['h' + str(k) for k in range(24)])

    if apply_adaptive_standardisation and not os.path.isfile(f"Results_py//dataset_{dataset.replace('.csv', '')}_larsmodel_LEAR_as_calibration_window_None.csv"):
        ...
        forecast.to_csv(f"Results_py//dataset_{dataset.replace('.csv', '')}_larsmodel_LEAR_as_calibration_window_None.csv")
        forecast = pd.DataFrame(index=df_test.index[::24], columns=['h' + str(k) for k in range(24)])
    else:
...
datasets = ["SP_2023.csv", "DE_2023.csv"]
apply_adaptive_standardisation_list = [False]
...
for apply_adaptive_standardisation in apply_adaptive_standardisation_list:
    for dataset in datasets:
        ...
        for calibration_window in calibration_windows:
            if apply_adaptive_standardisation:
                file_name = f"dataset_{dataset.replace('.csv', '')}_larsmodel_LEAR_as_calibration_window{calibration_window}.csv"
            else:
